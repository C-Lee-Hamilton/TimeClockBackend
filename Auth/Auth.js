...
router.get("/time-range", passport.authenticate("jwt"), async (req, res) => {
  try {
    const userId = req.user._id;
    ...
    const totalHours = parseFloat(
      (
        filteredEntries.reduce((sum, entry) => sum + entry.time, 0) / 60
      ).toFixed(2)
    );

    const totalPay = parseFloat(
      filteredEntries.reduce((sum, entry) => sum + entry.pay, 0).toFixed(2)
    );
    res.json({ success: true, totalHours, totalPay });
  } catch (err) {
    console.error(err);
    return res
      .status(500)
